...
impl ServerHandler for RuntimeMcpServer {
    fn get_info(&self) -> ServerInfo {
        ServerInfo::new(ServerCapabilities::builder().enable_tools().build())
            ...
            .with_instructions("Use credential_list first to choose an alias. For HTTP, call api_call with request_path only; target URLs stay hidden. For SQL, optional database selects another DB on the same registered Postgres server; run sql_schema before unknown tables, then sql_query with explicit columns/WHERE/count/group and avoid SELECT *. If body is omitted, follow omit_reason and more.options.next_action: add_jsonpath means inspect more.preview.paths and retry with 1-3 selected JSONPath paths; narrow_jsonpath means reduce the existing JSONPath scope; narrow_request means reduce the upstream request with target-native pagination/filter/selector/time range.")
    }
}
