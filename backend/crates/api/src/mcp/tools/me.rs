...
fn capabilities_for_toolset(toolset: McpToolset) -> Vec<Capability> {
    let specs = match toolset {
        McpToolset::Runtime => vec![
            ...
            ("api_call", "HTTP credential alias로 JSON API를 호출합니다."),
            (
                "sql_schema",
...
fn workflow_for_toolset(toolset: McpToolset) -> Vec<String> {
    match toolset {
        McpToolset::Runtime => vec![
            ...
            "credential_list로 alias와 policy를 확인합니다.".to_owned(),
            "credential category에 맞는 runtime 도구를 호출합니다.".to_owned(),
        ],
        McpToolset::Admin => vec![
            "me로 관리 capability와 credential_summary를 확인합니다.".to_owned(),
            "credential_register_*로 secret을 봉인해 등록합니다.".to_owned(),
            "credential_update_*로 metadata와 policy만 수정합니다.".to_owned(),
            "secret 교체나 target URL 변경은 credential_delete 후 재등록합니다.".to_owned(),
        ],
    }
}
...
mod tests {
    ...
    fn admin_me_reports_admin_capabilities_without_aliases() {
        ...
        assert!(
            out.capabilities
                .iter()
                .any(|capability| capability.tool == "credential_delete")
        );
    ...
    fn runtime_me_excludes_admin_capabilities_and_aliases() -> Result<(), serde_json::Error> {
        ...
        assert!(
            out.capabilities
                .iter()
                .all(|capability| capability.tool != "credential_delete")
        );
