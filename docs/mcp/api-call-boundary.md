...
jsonpath projection returns flat-keyed object; RFC 9535-compatible filters including `match()`/`search()` are supported; `.length()`/`.count()` suffix may return small scalar counts
