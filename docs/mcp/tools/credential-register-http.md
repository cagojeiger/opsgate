...
- `client_cert_pem`/`client_key_pem`은 mTLS 기반 target 인증입니다. PEM은 HTTP header에 들어가지 않고 TLS handshake에서 client identity로 사용됩니다. `client_key_pem`은 봉인(sealed)되며 절대 반환하지 않습니다.
...
- Auth, cookie, host, hop-by-hop, `X-Forwarded-*`, `Content-Type` header는
  계속 차단됩니다.
- `allow_insecure_transport=true`는 plain HTTP 허용용입니다. HTTPS 인증서 검증을 끄지 않으며, self-signed/private CA HTTPS target은 `tls_server_ca`를 제공해야 합니다.
